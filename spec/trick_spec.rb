...
module Euchre
  ...
  RSpec.describe Props::Trick do
    ...
    let(:ten_of_diamonds) { Props::Card.for(rank: TEN, suit: DIAMONDS) }
    let(:king_of_diamonds) { Props::Card.for(rank: KING, suit: DIAMONDS) }

    let(:ten_of_spades) { Props::Card.for(rank: TEN, suit: SPADES) }
    let(:jack_of_spades) { Props::Card.for(rank: JACK, suit: SPADES) }
    let(:ace_of_spades) { Props::Card.for(rank: ACE, suit: SPADES) }

    let(:nine_of_clubs) { Props::Card.for(rank: NINE, suit: CLUBS) }
    let(:jack_of_clubs) { Props::Card.for(rank: JACK, suit: CLUBS) }
    let(:ace_of_clubs) { Props::Card.for(rank: ACE, suit: CLUBS) }

    let(:joker) { Props::Card.for(rank: JOKER, suit: JOKER_SUIT) }

    context 'when clubs are trumps and all four players are playing' do
      let(:trumps) { CLUBS }
      let(:trick) { Props::Trick.new(trumps: trumps) }
    ...
    context 'when diamonds are trumps and three players are playing' do
      let(:trumps) { DIAMONDS }
      let(:trick) { Props::Trick.new(trumps: trumps, player_count: 3) }
