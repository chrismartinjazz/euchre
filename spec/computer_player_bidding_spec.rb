...
module Euchre
  ...
  RSpec.describe Players::ComputerPlayerBidding do
    before do
      allow_any_instance_of(Players::ComputerPlayerBidding).to receive(:think)
      allow_any_instance_of(Players::ComputerPlayerBidding).to receive(:wait_for_user_to_press_enter)
    end

    let(:computer_player_bidding) { Players::ComputerPlayerBidding.new }
    let(:ten_of_clubs) { Props::Card.for(rank: TEN, suit: CLUBS) }
    let(:ten_of_diamonds) { Props::Card.for(rank: TEN, suit: DIAMONDS) }
    let(:queen_of_hearts) { Props::Card.for(rank: QUEEN, suit: HEARTS) }
    ...
    context "with a strong clubs hand" do
      let(:hand) { [
        ...
        Props::Card.for(rank: ACE, suit: CLUBS),
        Props::Card.for(rank: ACE, suit: SPADES)
      ] }
    ...
    context 'with a borderline hearts hand (Jack DIAMONDS, Ten HEARTS, Ace SPACES, Jack CLUBS, Nine SPADES)' do
      let(:hand) { [
        ...
        Props::Card.for(rank: TEN, suit: HEARTS),
        Props::Card.for(rank: ACE, suit: SPADES),
        Props::Card.for(rank: JACK, suit: CLUBS),
    ...
    context 'with a hand with no off-trump singles' do
      let(:hand) { [
        ...
        Props::Card.for(rank: JACK, suit: SPADES),
        Props::Card.for(rank: JACK, suit: CLUBS),
        Props::Card.for(rank: KING, suit: HEARTS),
        Props::Card.for(rank: TEN, suit: HEARTS)
      ] }
